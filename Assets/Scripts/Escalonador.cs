...
public class Escalonador : MonoBehaviour
{
    ...
    void FixedUpdate() // Este eh o loop do simulador, executado a cada unidade de tempo.
    {
        ...
        if(CPU2 != null)Executar(CPU2, 2); else ecp.Despachar(2);
        if(CPU3 != null)Executar(CPU3, 3); else ecp.Despachar(3);
        if(CPU4 != null)Executar(CPU4, 4); else ecp.Despachar(4);
        t += 1;
    }
    ...
    List<Processo> LerEntrada() {
        ...
        ar sr = new StreamReader(Application.dataPath + "/" + Teste.txt);
        var fileContents = sr.ReadToEnd();
        sr.Close();
 
        var lines = fileContents.Split(","[0]);
        for (int i = 0; i<lines;5) {
            nomeProcesso = GeraNome(i);
            Processo nomeProcesso = new Processo(lines[i],lines[i+1],lines[i+2],lines[i+3],lines[i+4]);
            falae.Add(nomeProcesso);
         }
