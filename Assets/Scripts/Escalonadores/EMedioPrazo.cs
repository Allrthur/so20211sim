...
public class EMedioPrazo
{
    ...
    public (int,int) LiberarMP(int memItem, int prioridade) // Liberar MP pede a memoria necessaria para admitir o novo processo
    {
        ...
        Debug.Log("Função para liberar memoria devido a falta de espaço chamada");
        ...
        if(Filas.fila_pronto_p1_rq2.Count > 0){
            ...
            if(Filas.fila_pronto_p1_rq2[maior].GetMem() >= memItem){                // checa se maior processo dessa lista é o suficiente para liberar espaço para o novo
                Suspensos.fila_pronto_p1_rq2.Add(Filas.fila_pronto_p1_rq2[maior]);  // coloca ele no suspenso
                ...
                Debug.Log("Liberou o maior do rq2");
            ...
            else{ Debug.Log("Tamanho do maior processo no rq2 nao foi suficiente");}
        }
        if(Filas.fila_pronto_p1_rq1.Count > 0){                                     //logica se repete para outras filas
            ...
            if(Filas.fila_pronto_p1_rq1[maior].GetMem() >= memItem){ 
                ...
                Debug.Log("Liberou o maior do rq1");
                return (retorno, 1);
            }
            else{ Debug.Log("Tamanho do maior processo no rq1 nao foi suficiente");}
        }
        if(Filas.fila_pronto_p1_rq0.Count > 1 || prioridade == 0){ // Maior que 1 na checagem da rq0 pra nao deixar ficar um ciclo infinito caso seja prioridade 1, se for prioriade 0 vai dar liberar de qualquer forma
